...
class _EditUniversityPostScreenState extends ConsumerState<EditUniversityPostScreen> {
  ...
  Future<void> _save() async { if (_title.text.trim().isEmpty || _content.text.trim().isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Title and content are required.'))); return; } await ref.read(communityRepositoryProvider).updateUniversityPost('prof_org_01', widget.post.copyWith(title: _title.text.trim(), content: _content.text.trim(), updatedAt: DateTime.now())); if (mounted) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Post updated successfully'))); Navigator.pop(context, true); } }
