...
class _UniversityCreatePostScreenState extends ConsumerState<UniversityCreatePostScreen> {
  ...
  Future<void> _publish() async { if (_title.text.trim().isEmpty || _content.text.trim().isEmpty || _category == null) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Complete title, content, and category.'))); return; } final auth = ref.read(authControllerProvider); final organizationId = auth.currentUser?.id; final organizationName = auth.organizationProfile?.orgName ?? auth.currentUser?.name ?? ''; if (organizationId == null) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please sign in first.'))); return; } await ref.read(communityRepositoryProvider).createUniversityPost(universityId: organizationId, universityName: organizationName, title: _title.text.trim(), content: _content.text.trim(), topic: _category!); if (mounted) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('University post published'))); Navigator.pop(context, true); } }
