...
void main() {
  test('university opportunity is visible to students and application reaches university', () async {
    ...
    expect(
      (await opportunities.getOpportunities()).any(
        (item) => item.id == created.id,
      ),
      isTrue,
    );
    await opportunities.markAsApplied(created.id);
    final applicants = await MockApplicationRepository.instance.forOpportunity(
      created.id,
    );
    ...
    await MockApplicationRepository.instance.updateStatus(
      'cross-role-university',
      applicants.single.id,
      ApplicationStatus.shortlisted,
    );
    expect(
      (await MockApplicationRepository.instance.forStudent('usr_student_01'))
          .single
          .status,
      ApplicationStatus.shortlisted,
    );
  ...
  test(
    'unpublished university opportunity is excluded from student discovery',
    () async {
      final opportunities = MockOpportunityService();
      final created = await opportunities.createUniversityOpportunity(
  ...
  test(
    'university posts are in the student feed and student cannot delete them',
    () async {
      ...
      expect(
        (await community.getPosts()).any((item) => item.id == post.id),
        isTrue,
      );
