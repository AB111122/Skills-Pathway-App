...
void main() {
  group('Opportunities CRUD & Student Discovery Suite', () {
    ...
    test('University can create opportunities across all supported types', () async {
      final types = [
    ...
    test('Opportunity mutation and deletion enforce strict organization ownership', () async {
      ...
      expect(
        () => universityRepo.deleteOpportunity('org_other_university', created.id),
        throwsStateError,
      );

      // Delete by owner succeeds
      await universityRepo.deleteOpportunity('org_fast_01', created.id);
      final remaining = await universityRepo.getOwnedOpportunities('org_fast_01');
      expect(remaining.any((item) => item.id == created.id), isFalse);
    });

    test('Student discovery filters out draft, closed, and expired listings', () async {
      ...
      await opportunityService.closeUniversityOpportunity('org_nust_01', closed.id);
    ...
    test('Filters work correctly by category, verified status, and keyword', () async {
      final scholarshipList = await opportunityService.getOpportunities(
        filter: const OpportunityFilterModel(opportunityType: OpportunityType.scholarship),
      );
