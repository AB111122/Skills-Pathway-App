...
void main() {
  group('Student skills', () {
    test('normalizes, deduplicates, persists, and restores custom skills', () async {
      ...
      expect(updated.skills, ['Java', 'Networking', 'Automotive Diagnostics']);

      await auth.logout();
      await auth.login(email: 'student@example.com', password: 'Password123');
      final restored = await auth.getStudentProfile(user.id);
      expect(restored?.skills, containsAll(['Java', 'Networking', 'Automotive Diagnostics']));
    });
  ...
  group('AI service', () {
    test('sends the actual prompt and history and returns the model response', () async {
      ...
      final response = await service.sendMessage(
        ...
        null,
        history: const [
          ChatTurn(fromUser: true, content: 'I am interested in cybersecurity.'),
          ChatTurn(fromUser: false, content: 'Start with networking fundamentals.'),
        ],
      );

      expect(response, 'A networking roadmap');
      final messages = request['messages'] as List<dynamic>;
      expect(messages.any((item) => item['content'] == 'What skills should I learn?'), isTrue);
      expect(messages.any((item) => item['content'].toString().contains('cybersecurity')), isTrue);
    });
  });
}
